@php
    ...
    $btnLabel    = $isWhatsapp ? '🟢 WhatsApp ile Sipariş Ver' : ($block->data['button_label'] ?? 'Ürünü İncele');
@endphp
...
<div class="overflow-hidden transition-all duration-300 theme-card theme-card-stacked group relative {{ $variantClass }}">
    ...
    <div class="p-4 space-y-3">
        ...
        <a href="{{ $btnHref }}"
           ...
           class="inline-flex w-full items-center justify-center gap-2 rounded-[inherit] border border-current/10 px-4 py-3 text-sm font-semibold transition-opacity hover:opacity-90 {{ $isWhatsapp ? 'bg-green-500 text-white border-green-600' : '' }}">
            @if($isWhatsapp)
