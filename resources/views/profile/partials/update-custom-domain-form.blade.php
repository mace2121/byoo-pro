<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __('Profilinize ait özel bir alan adı bağlayın (Örn: links.sitem.com).') }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="custom_domain" :value="__('Özel Alan Adı')" />
        ...
        @if($user->profile?->custom_domain)
            <div class="p-4 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600">
                <h4 class="text-sm font-bold text-gray-700 dark:text-gray-200 mb-2">DNS Kurulumu:</h4>
                <p class="text-xs text-gray-600 dark:text-gray-400">Alan adınızı buraya yönlendirmek için DNS yönetiminize gidin ve aşağıdaki A kaydını ekleyin:</p>
                ...
                <div class="mt-4 flex items-center">
                    ...
                    @else
                        ...
                        <p class="ml-2 text-[10px] text-gray-500">(Yönlendirme tamamlandığında admin onayından sonra aktif olacaktır)</p>
                    @endif
                </div>
            </div>
        @endif
